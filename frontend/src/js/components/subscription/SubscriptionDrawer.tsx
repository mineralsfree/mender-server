...
export const SubscriptionDrawer = (props: SubscriptionDrawerProps) => {
  ...
  const canShowConfirmation = successConfirmationShown && previewPrice && orderedProducts && orderedAddons;

  const willLogout = orderedAddons?.length > enabledAddons?.length || currentPlanId !== selectedPlan.id;
